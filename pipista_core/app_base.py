...
class PipistaApp(ui.View):
    ...
    def _ensure_browser_overlay(self):
        """
        Create a fresh embedded browser when one is needed.

        Closing Pipista's browser destroys the WebView instead of keeping a
        hidden native browser alive between pages.
        """
        if getattr(self, 'browser_overlay', None) is not None:
            return
    ...
    def _close_browser(self, sender=None):
        """
        Tear down the embedded browser completely.

        A fresh ui.WebView is created on the next open.
        """
        overlay = getattr(
            self,
            'browser_overlay',
        ...
        self.browser_webview = None
        self.browser_title_label = None
        self.browser_close_button = None
        self.browser_header = None
        self.browser_overlay = None
